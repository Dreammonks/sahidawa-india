...
call, 1 store API page, up to 5 alert PDFs.
"""
...
COUNTED_TABLES = ["medicines", "pharmacies", "cdsco_reference", "drug_alerts", "etl_failed_rows"]
CDSCO_PAGE_SIZE = 50
ALERT_PDFS_TO_TRY = 5
...
def cdsco_alert_pdf() -> dict:
    headers = {"User-Agent": BROWSER_UA}
    page = requests.get(CDSCO_ALERTS_URL, headers=headers, timeout=30)
    page.raise_for_status()
    soup = BeautifulSoup(page.text, "html.parser")
    links = [urljoin(CDSCO_ALERTS_URL, a["href"]) for a in soup.find_all("a", href=True)
             if a["href"].lower().endswith(".pdf") or "download_file_division.jsp" in a["href"].lower()]
    if not links:
        raise RuntimeError("No PDF links found on the CDSCO alerts page")

    checked = []
    for link in links[:ALERT_PDFS_TO_TRY]:
        content = requests.get(link, headers=headers, timeout=60).content
        if b"<iframe" in content.lower():
            iframe = BeautifulSoup(content, "html.parser").find("iframe")
            real_url = urljoin(CDSCO_ALERTS_URL, iframe["src"]).replace(" ", "%20")
            content = requests.get(real_url, headers=headers, timeout=60).content
        with pdfplumber.open(BytesIO(content)) as pdf:
            text = "\n".join(page.extract_text() or "" for page in pdf.pages[:2])
        checked.append({"kb": len(content) // 1024, "text_chars": len(text.strip())})
        if text.strip():
            return {"pdf_links_on_page": len(links), "pdfs_checked": checked,
                    "text_preview": " ".join(text.split())[:300],
                    "note": "AI extraction into drug_alerts not run (no Gemini/Groq key); nothing inserted"}
    return {"pdf_links_on_page": len(links), "pdfs_checked": checked,
            "note": "All checked PDFs are scanned images (no text layer); needs OCR/AI to extract alerts"}
