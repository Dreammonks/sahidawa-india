...
describe("GET /api/v1/products/barcode/:gtin", () => {
    ...
    it("returns 400 for a bad check digit with no matching medicine, without logging it", async () => {
    ...
    it("matches a seed medicine whose non-GS1 barcode fails the check digit", async () => {
        repo.findMedicineByBarcode.mockResolvedValue(
            medicineFixture({ is_cdsco_verified: true, is_counterfeit_alert: true })
        );

        const res = await lookup("8901111111111");

        expect(res.status).toBe(200);
        expect(res.body.status).toBe("found");
        expect(res.body.product.category).toBe("allopathic");
        expect(res.body.product.regulator).toBe("CDSCO");
        expect(res.body.medicine.brand_name).toBe("Augmentin 625 Duo");
