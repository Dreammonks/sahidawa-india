...
export const productLookupService = {
     ...
     *
     * Codes failing the GS1 check digit skip the catalogue and the review queue (a
     * typo should not be logged as a new product) but are still matched against
     * medicines.barcode_id, because the project's seed data uses non-GS1 codes.
     */
    async lookupByBarcode(gtin: string): Promise<ProductLookupResult> {
        ...
        if (!isValidGtin(gtin)) {
            ...
            if (medicine) return productFromMedicine(gtin, medicine);
            return {
                status: "invalid",
                gtin,
